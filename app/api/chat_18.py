...
async def websocket_chat(
    ...
    try:
        while True:
            ...
            # Save user message
            try:
                await save_user_message(db, chat_id, text, Message18)
            except Exception:
                log.exception("[WS %s] Failed to save user message", chat_id)

            # Moderation check
            try:
                context = await get_message_context(db, chat_id, Message18)
...
async def chat_audio(
    ...
    try:
        ...
        ai_reply = await get_ai_reply_via_websocket_18(
            chat_id,
            transcript_text,
            influencer_id,
            user_id,
...
async def get_ai_reply_via_websocket_18(
    chat_id: str,
    message: str,
    influencer_id: str,
    user_id: int,
    ...
    """
    Get AI reply for 18+ audio chat.
    
    This is a thin wrapper around handle_turn_18 for the audio endpoint.
    """
    if not user_id:
