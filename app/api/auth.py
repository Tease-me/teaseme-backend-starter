...
async def login(data: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    access_minutes = settings.ACCESS_TOKEN_EXPIRE_MINUTES_REMEMBER if data.remember_me else settings.ACCESS_TOKEN_EXPIRE_MINUTES
    refresh_days = settings.REFRESH_TOKEN_EXPIRE_DAYS_REMEMBER if data.remember_me else settings.REFRESH_TOKEN_EXPIRE_DAYS

    access_token = create_token(
        {"sub": str(user.id), "rm": data.remember_me},
        settings.SECRET_KEY,
...
async def refresh_token(refresh_token: str, db: AsyncSession = Depends(get_db)):
    ...
    access_minutes = settings.ACCESS_TOKEN_EXPIRE_MINUTES_REMEMBER if remember_me else settings.ACCESS_TOKEN_EXPIRE_MINUTES
    refresh_days = settings.REFRESH_TOKEN_EXPIRE_DAYS_REMEMBER if remember_me else settings.REFRESH_TOKEN_EXPIRE_DAYS

    new_access_token = create_token(
        {"sub": str(user.id), "rm": remember_me},
        settings.SECRET_KEY,
        timedelta(minutes=access_minutes),
    )
